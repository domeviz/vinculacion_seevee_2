...
<template>
  <v-sheet>
        ...
        <v-card outlined color="transparent" width="100%">
          <v-row no-gutters>
            ...
            <v-col cols="12" md="3">
              ...
              <!--Boton Busqueda-->
              <v-fab-transition>
                <v-btn active-class="deep-purple " color="#12A2C2" dark absolute left fab @click.stop="drawer = !drawer">
                  <v-icon>search</v-icon>
                </v-btn>
              </v-fab-transition>
              <!--Boton Limpiar-->
              <v-fab-transition>
                <v-btn  active-class="deep-purple " color="#12A2C2" dark absolute right fab
                  @click.stop=limpiar()>
    ...
    <v-navigation-drawer v-model="drawer" absolute temporary width="350px" color="#12A2C2">
      <v-row class="filtro">
        <v-col cols="12">
          ...
          <div>
            <v-select v-model="type" :items="types" dense outlined hide-details class="ma-5"></v-select>
            <span>Seleccionado: {{ type }}</span>
          </div>
          <div class="txt">PARTIDO POLITICO*</div>
          <div>
            <v-select v-model="partido" :items="arregloPartido" dense outlined hide-details class="ma-5"></v-select>
          ...
          <div>
            <v-select v-model="select" :items="provs" dense outlined hide-details class="ma-5"></v-select>
            <span>Seleccionado: {{ select }}</span>
          </div>

          <div class="txt">CANTÓN</div>
          <v-select v-model="select2" :items="cantons" dense outlined hide-details class="ma-5"></v-select>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      tituloMapa: '0',
      titulo: 'Resultados Nacionales 2021'
  ...
  methods: {
    ...
    datosBusqueda2() {
      ...
      if (this.select == "") {
        let candidato = this.select == "" ? this.id_codMapa = [varVuelta, condicionPartido, "", ""] : '';
        console.log(candidato)
        this.drawer = false;
      } else if (this.select2 == "") {
        const resultado = this.arreglo_prov.find(varProvincia => varProvincia.provincia === this.select);
        ...
        const cantonSelect = arregloCantones.find(varCanton => varCanton.nombre === this.select2);
    ...
    asignarTitulo() {
      const textoTitulo = this.arreglo_prov.find(datos => datos.id === this.id_codMapa[2]);
      const arregloTexto = textoTitulo.cantones;
      const textCanton = arregloTexto.find(datosCanton => datosCanton.idcan === this.id_codMapa[3])
    ...
    selectProv() {
      ...
      if (this.type == '1' || this.type == '2') {
        let condicionPartido = this.type == '1' ? this.arregloPartido = this.auxpartido : this.arregloPartido = this.auxpartido2;

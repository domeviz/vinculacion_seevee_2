...
export default {
  ...
  methods: {
    async buscarAusentes2() {
      const data = await muestraVotos("presi2v");
      this.ausentes = data;
    },
    async buscarGenero2() {
      const data = await totalGenero("presi2v", "F");
      this.femenino = data;
      const data2 = await totalGenero("presi2v", "M");
      this.masculino = data2;


    },
    async buscarSufragantes2() {
      const data = await totalSufragio("presi2v");
      this.sufragantes = data;

    },
  },
  mounted() {
    this.buscarAusentes2();
    this.buscarSufragantes2();
    this.buscarGenero2();
